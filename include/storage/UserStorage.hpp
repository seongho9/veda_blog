...
namespace storage
{
    ...
    class UserStorageODBC : public UserStorage, public utils::Singleton<UserStorageODBC>
    {
    private:
        utils::DBConnection* connectionPool;

        UserStorageODBC();
        friend class utils::Singleton<UserStorageODBC>;

    public:
        int insert_user(domain::User user);
        int find_user_byid(domain::User* user, std::string userid);
        int delete_user_byid(std::string userid);
        int get_post_byuser(std::vector<domain::Post>* post, std::string user_id);
        int get_comment_byuser(std::vector<domain::Comment>* comment, std::string user_id);
